...
class Mechanism:
    ...
    def solve_with_cvar(self, tau: float, prob_state0: float, threshold_index: int, do_print=False):
    ...
    def solve_no_infdo(self, tau: float, prob_state0: float, threshold_index: int, do_print=False):
        ...
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env) as model:
                ...
                # model.addConstr(allocations[:25] == -1)
                # model.addConstr(allocations[25:] == 1 / 3)
    ...
    def solve_no_thresh(self, tau: float, prob_state0: float, threshold_index: int, do_print=False):
        """
    ...
    def solve_virtuals(self, tau: float, prob_state0: float, threshold_index: int, do_print=False):
        """
        With this formualtion, when the exteranlity outweights the transfer, or we cross the virtual values,
        the problem becomes unbounded.
        """
        ...
        virtual_values = VirtualValues(self.dist, self.midpoints, tau, prob_state0, iron=True)
            ...
            with gp.Model(env=env) as model:
                ...
                for i, type in enumerate(self.midpoints):
                    ...
                    if i > 0:  # Dont think we need this with ironed virtual values
                        ...
                        # if i >= 55:
                        #     virtual_value = virtual_values.positive[i]
                        # else:
                        #     virtual_value = virtual_values.negative[i]

                        virtual_value = (
                            virtual_values.negative[i] * (1 - binaries[i])
                            + virtual_values.positive[i] * binaries[i]
                        )
                        objective_value += (virtual_value - 73.67) * allocations[i]
                        ...
                        # extra_ex = tau * prob_state0 * self._pdfs[i]
                ...
                transfers = self._allocations_to_transfers(allocations)
                externalities = self._allocations_to_externalities(allocations, tau, prob_state0)
                multiplier = 1  # model.getConstrByName("integral").Pi * self.num_intervals
