...
class VirtualValues:
    ...
    def _unironed_positive_values(self):
        return self.pdf(self.types) * (
            self.types - 1 + self.alpha * (1 - self.prob_state0)
        ) + self.cdf(self.types)
    ...
    def _ironed_positive_values(self):
        integral = self.cdf(self.types) * (self.types + self.alpha * (1 - self.prob_state0) - 1)
        return self._iron(integral)

    def _ironed_negative_values(self):
        integral = self.cdf(self.types) * (self.types + self.alpha * self.prob_state0)
        return self._iron(integral)

    def _iron(self, integral: _Floats) -> _Floats:
        envelope = convex_envelope(self.types, integral)
        g = np.gradient(envelope, 1 / len(self.types))
        return g
