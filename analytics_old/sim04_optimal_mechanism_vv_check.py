...
def main():
    ...
    noinfo_allocations = np.zeros(len(mechanism.midpoints))
    noinfo_allocations[:50] = -1
    noinfo_allocations[50:] = 1
    ...
    print(np.mean(allocations), np.mean(allocations_vv))

    # fig, ax = plt.subplots(figsize=(4.5, 3), sharey=False)
    # ax.plot(midpoints, binaries, color="red")
    # ax2 = ax.twinx()
    # ax2.plot(midpoints, allocations, color="blue")
    # fig.tight_layout()
    # fig.savefig(savedir / f"binaries.pdf", dpi=300)

    # ms = []
    # objs = []
    # for m in np.linspace(-0.3, -0.1, 100):
    #     (
    #         *_,
    #         o,
    #         _,
