...
def main():
    ...
    (allocations, transfers, *_) = mechanism.solve(tau, prob_state0, threshold_index, do_print=True)
    print("--")
    (allocations, transfers, *_) = mechanism.solve(tau, prob_state0, 50, do_print=True)

    from scipy.integrate import trapezoid

    avg = trapezoid(transfers * dist.pdf(mechanism.midpoints), x=mechanism.midpoints)
    print(avg)

    fig, ax = plt.subplots()
    ax.plot(mechanism.midpoints, transfers)
    fig.savefig(savedir / "transfers.pdf")

    fig, ax = plt.subplots()
    ax.plot(mechanism.midpoints, allocations)
